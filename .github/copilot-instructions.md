...
## Project Structure (Current - Optimized)
```
/FINAL_YEAR_PROJECT/
├── config/
│   ├── camera_config.json      # Camera: width, height, fps
│   ├── control_config.json     # PI gains, setpoints, pump params
│   └── vision_config.json      # Bubble detection thresholds
├── src/
│   ├── vision/                 # CONSOLIDATED (2 files, 275 lines)
│   │   ├── vision_processor.py # Unified: camera, preprocessing, detection, analysis
│   │   └── __init__.py
│   ├── control/                # CONSOLIDATED (2 files, 232 lines)
│   │   ├── hardware_controller.py  # Unified: PI control, pump driver, safety
│   │   └── __init__.py
│   ├── ml/                     # OPTIONAL (anomaly detection)
│   │   ├── anomaly_detector.py # Isolation Forest (not integrated)
│   │   └── __init__.py
│   ├── api/                    # OPTIMIZED (4 files, 482 total lines)
│   │   ├── main.py             # FastAPI app, lifecycle (164 lines)
│   │   ├── routes.py           # REST endpoints (195 lines)
│   │   ├── websocket.py        # Real-time streaming (123 lines)
│   │   └── __init__.py
│   └── utils/                  # OPTIONAL (logging, data storage)
│       ├── logger.py
│       ├── data_manager.py
│       └── __init__.py
├── dashboard/                  # SIMPLIFIED (no charts)
│   ├── index.html              # Main dashboard (video, metrics, controls)
│   ├── css/
│   │   └── styles.css
│   └── js/
│       ├── app.js              # Dashboard logic
│       ├── video_stream.js     # WebSocket handler
│       └── charts.js           # DEPRECATED (removed functionality)
...
### 1. Vision Processing (src/vision/vision_processor.py - 275 lines)

**CONSOLIDATED MODULE** - Single file replaces camera.py, preprocessor.py, bubble_detector.py, froth_analyzer.py

**Image Preprocessing Pipeline**:
```python
# All in VisionProcessor.process_bubbles() method
1. Convert to grayscale (reduce 3x data)
...
```python
# VisionProcessor.get_metrics() returns:
{
    'bubble_count': int,           # Number of bubbles detected
    'avg_bubble_size': float,      # Average bubble area (pixels²)
    'size_std_dev': float,         # Bubble size std deviation
    'froth_coverage': float,       # 0-1 bubble coverage ratio
    'froth_stability': float,      # 0-1 stability score
    'timestamp': str,              # ISO format timestamp
    'success': bool                # Processing success flag
}
```

**Detection Parameters** (tunable class constants):
- `MIN_BUBBLE_AREA = 80` pixels²
- `MAX_BUBBLE_AREA = 5000` pixels²
- `DISTANCE_THRESHOLD = 0.125` (watershed sensitivity)
- `MIN_CIRCULARITY = 0.45` (shape filter)

### 2. Hardware Control (src/control/hardware_controller.py - 232 lines)

**CONSOLIDATED MODULE** - Single file with dual-mode pump control

**Control Architecture**:
```python
class HardwareController:
    # Dual-mode peristaltic pump (GPIO 12)
    - Auto mode: PI controller adjusts based on bubble count
    - Manual mode: Direct PWM duty cycle control
    
    # Always-manual motors (GPIO 13-15)
    - Agitator, air pump, feed pump: Direct PWM only
    
    # Safety: E-Stop (GPIO 22), max duty 80%
```

**PI Controller** (simplified, no D term):
```python
def _pi_update(self, measured_value: float) -> float:
    error = self.pi_setpoint - measured_value
    self.pi_integral += error
    self.pi_integral = max(-50, min(50, self.pi_integral))  # Anti-windup
    
    p_term = self.pi_kp * error
    i_term = self.pi_ki * self.pi_integral
    output = p_term + i_term
    
    return max(0.0, min(self.max_pump_duty, output))
```

**Default PI Parameters**:
- Kp = 0.5 (proportional gain)
- Ki = 0.05 (integral gain)
- Setpoint = 120 bubbles
- Max pump duty = 80%
